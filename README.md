...
对于登录后的 SaaS 工作台、CRM、流程和数据系统，它还会根据任务路由参考源：用 Mobbin/SaaSUI 观察真实产品流程，用 Cloudscape、Carbon、Atlassian、Fluent 2 和 Ant Design 研究表格、筛选、抽屉、批量操作和状态反馈。Mobbin MCP 是可选增强，不是安装或设计前提；Refero 继续用于营销页和产品介绍页。
...
| SaaS 工作台 | 根据任务选择集合、筛选、详情、抽屉、弹窗和批量操作，并覆盖空、加载、错误、权限、冲突与成功状态 |
...
- 增加企业 SaaS 工作台页面路由，区分工作台、数据集合、详情、配置和协作界面。
- 增加表格、筛选、详情抽屉、弹窗、批量操作与完整状态矩阵的判断规则。
- 将 Mobbin、SaaSUI、Cloudscape、Carbon、Atlassian、Fluent 2、Ant Design 和 Refero 按任务分工纳入可选参考源，不强制付费服务或特定框架。
- 保留外部参考降级、品牌与代码借鉴边界，以及首次开发、重构和多轮迭代的通用范围。
